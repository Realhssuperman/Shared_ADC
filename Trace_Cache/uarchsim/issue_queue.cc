...
bool issue_queue::stall(unsigned int bundle_inst) {
  ...
  ifprintf(logging_on,
           proc->dispatch_log,
           "ISSUE_QUEUE: Checking for stall fl_length: %u  length: %u bundle_inst: %u stall: %s\n",
           fl_length,
           length,
           bundle_inst,
           (fl_length < bundle_inst) ? "true" : "false");
...
void issue_queue::dump_iq(pipeline_t *proc, unsigned int index, FILE *file) {
  proc->disasm(proc->PAY.buf[q[index].index].inst,
               proc->cycle,
               proc->PAY.buf[q[index].index].pc,
               proc->PAY.buf[q[index].index].sequence,
               file);
  ...
  ifprintf(logging_on, file, "branch_mask: %"
      PRIu64
      "\t", q[index].branch_mask);
