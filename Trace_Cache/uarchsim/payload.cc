...
void payload::dump(pipeline_t *proc, unsigned int index, FILE *file) {
  ...
  ifprintf(logging_on, file, "next_pc    : %"
      PRIxreg
      "\t", buf[index].next_pc);
  ifprintf(logging_on, file, "c_next_pc  : %"
      PRIxreg
      "\t", buf[index].c_next_pc);
  ifprintf(logging_on, file, "Mem addr   : %"
      PRIxreg
      "\t", buf[index].addr);
  ...
  ifprintf(logging_on, file, "RS1 Value  : 0x%"
      PRIxreg
      "\t", buf[index].A_value.dw);
  ifprintf(logging_on, file, "RS1 Value  : %"
      PRIsreg
      "\t", buf[index].A_value.sdw);
  ...
  ifprintf(logging_on, file, "RS2 Value  : 0x%"
      PRIxreg
      "\t", buf[index].B_value.dw);
  ifprintf(logging_on, file, "RS2 Value  : %"
      PRIsreg
      "\t", buf[index].B_value.sdw);
  ifprintf(logging_on, file, "RS2 Value  : %f\t", buf[index].B_value.d);
  ifprintf(logging_on, file, "\n");
  ifprintf(logging_on, file, "U   Imm    : 0x%"
      PRIxreg
      "\t", buf[index].inst.u_imm());
  ifprintf(logging_on, file, "\n");
  if (unlikely(buf[index].D_valid)) {
    ...
    ifprintf(logging_on, file, "RS3 Value  : 0x%"
        PRIxreg
        "\t", buf[index].D_value.dw);
    ifprintf(logging_on, file, "RS3 Value  : %"
        PRIsreg
        "\t", buf[index].D_value.sdw);
  ...
  ifprintf(logging_on, file, "RD  Value  : 0x%"
      PRIxreg
      "\t", buf[index].C_value.dw);
  ifprintf(logging_on, file, "RD  Value  : %"
      PRIsreg
      "\t", buf[index].C_value.sdw);
