...
void pipeline_t::writeback(unsigned int lane_number) {
  ...
  if (Execution_Lanes[lane_number].wb.valid) {
    ...
    // * Squash instructions in the pipeline that are logically after the branch -- those instructions
    ...
    if (PAY.buf[index].checkpoint) {
      ...
      if (PERFECT_BRANCH_PRED) {
        ...
        assert(
            (PAY.buf[index].next_pc == PAY.buf[index].c_next_pc) || !PAY.buf[index].good_instruction || SPEC_DISAMBIG);
        ...
        REN->resolve(PAY.buf[index].AL_index, PAY.buf[index].branch_ID, true);
        this->resolve(PAY.buf[index].branch_ID, true);
        ...
        BP.fix_pred(PAY.buf[index].pred_tag,
                    PAY.buf[index].c_next_pc);    // Roll-back the branch predictor to the point of the resolved branch.
        ...
        // * Recover key units: the Fetch Unit, the renamer module (RMT, FL, AL), and the LSU.
        //   TODO: The Fetch Unit is recovered in the statements immediately preceding this comment (please study them for knowledge).
        //   TODO: The LSU is recovered in the statements immediately following this comment (please study them for knowledge).
        ...
        //    * See pipeline.h for details about the two arguments of resolve().
        this->resolve(PAY.buf[index].branch_ID, false);
