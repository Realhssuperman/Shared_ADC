...
static void print_cache_config(FILE *fp,
                               unsigned int sets,
                               unsigned int assoc,
...
#undef OPEN_LOG_FILE
  ...
  TC = new trace_cache(
    ...
    TRACE_CACHE_ASSOC,
    TRACE_CACHE_MAX_ONGOING_FILL,
    TRACE_CACHE_NON_BLOCK_FILL,
    TRACE_INDEX_WITH_PRED_BIT,
    TRACE_CACHE_TERM_HEURISTIC,
    mmu
    );
    ...
    for (j = 0; j < (unsigned int) NUMBER_FU_TYPES; j++) {
      if (fu_lane_matrix[j] & (1 << i)) {
        if (ex_depth == 0) {
          ...
          printf(
              "Adjust fu_lane_matrix[] and/or fu_lat[] so that a given lane has a single latency for all its FU types.\n");
          exit(-1);
        }
      }
    }
...
void pipeline_t::disasm(insn_t insn) {
  ...
  ifprintf(logging_on, stderr, "core %3d: 0x%016"
      PRIx64
      " (0x%08"
      PRIx64
      ") %s\n",
