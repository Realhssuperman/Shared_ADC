...
void pipeline_t::decode() {
  ...
  for (i = 0; i < fetch_width; i++) {
    ...
    LOG(decode_log, cycle, PAY.buf[index].sequence, PAY.buf[index].pc, "Instruction: %08"
        PRIX32
        "", (word_t) inst.bits());
    ...
    switch (inst.opcode()) {
      case OP_JAL:
      case OP_JALR:
      case OP_BRANCH: PAY.buf[index].checkpoint = true;
        break;

      default: PAY.buf[index].checkpoint = false;
        break;
    }
    ...
    switch (inst.opcode()) {
      ...
      case OP_JAL:
      case OP_JALR: PAY.buf[index].flags = (F_CTRL | F_UNCOND);
        PAY.buf[index].fu = FU_BR;
        break;

      case OP_BRANCH: PAY.buf[index].flags = (F_CTRL | F_COND);
        PAY.buf[index].fu = FU_BR;
        break;

      case OP_LOAD: PAY.buf[index].flags = (F_MEM | F_LOAD | F_DISP);
        PAY.buf[index].fu = FU_LS;
        break;

      case OP_STORE: PAY.buf[index].flags = (F_MEM | F_STORE | F_DISP);
        PAY.buf[index].fu = FU_LS;
        break;
      ...
      case OP_LUI:
      case OP_AUIPC: PAY.buf[index].flags = (F_ICOMP);
        PAY.buf[index].fu = FU_ALU_S;
        break;

        // Set both F_MEM and F_FMEM so that they go to the MEM lane
        // but the FP unit requirement also gets checked in DISPATCH.
      case OP_LOAD_FP: PAY.buf[index].flags = (F_MEM | F_FMEM | F_LOAD | F_DISP);
        PAY.buf[index].fu = FU_LS_FP;
        break;

        // Set both F_MEM and F_FMEM so that they go to the MEM lane
        // but the FP unit requirement also gets checked in DISPATCH.
      case OP_STORE_FP: PAY.buf[index].flags = (F_MEM | F_FMEM | F_STORE | F_DISP);
        PAY.buf[index].fu = FU_LS_FP;
        break;
      ...
      case OP_MSUB:
      case OP_NMADD:
      case OP_NMSUB: PAY.buf[index].flags = (F_FCOMP);
...
//      	    break;
      ...
      case OP_AMO:
        switch (inst.funct5()) {
          case FN5_AMO_LR: PAY.buf[index].flags = (F_MEM | F_LOAD | F_DISP | F_AMO);
            PAY.buf[index].fu = FU_LS;
            break;
          case FN5_AMO_SC: PAY.buf[index].flags = (F_MEM | F_STORE | F_DISP | F_AMO);
            PAY.buf[index].fu = FU_LS;
            break;
          default: PAY.buf[index].flags = (F_AMO);
            PAY.buf[index].fu = FU_ALU_S;
            //assert(0);
        }
        break;
    ...
    switch (inst.opcode()) {
      ...
      case OP_MISC_MEM: PAY.buf[index].iq = SEL_IQ_NONE;
        break;
