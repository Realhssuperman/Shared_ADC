...
bool lsu::stall(unsigned int bundle_load, unsigned int bundle_store) {
  ...
  ifprintf(logging_on,
           proc->dispatch_log,
           "LSQ: Checking for stall lq_length: %u  sq_length: %u bundle_load: %u bundle_store: %u load_stall: %s store_stall %s\n",
           lq_length,
           sq_length,
           bundle_load,
           bundle_store,
           load_stall ? "true" : "false",
           store_stall ? "true" : "false");
...
void lsu::store_addr(cycle_t cycle,
  ...
  if (!PERFECT_DCACHE) {
    ...
    if (SQ[sq_index].miss_resolve_cycle == INVALID_CYCLE) inc_counter(store_mhsr_miss_count);
  }
...
bool lsu::load_addr(cycle_t cycle,
  ...
  if (!PERFECT_DCACHE) {
    ...
    if (LQ[lq_index].miss_resolve_cycle == INVALID_CYCLE) {
...
bool lsu::load_unstall(cycle_t cycle, unsigned int &pay_index, reg_t &value) {
  ...
  while (!((scan == lq_tail) && (scan_phase == lq_tail_phase)) && !unstalled) {
    ...
    if (LQ[scan].addr_avail && !LQ[scan].value_avail) {
      ...
      if (!PERFECT_DCACHE && (LQ[scan].miss_resolve_cycle == INVALID_CYCLE)) {
        bool hit;
...
void lsu::execute_load(cycle_t cycle,
  ...
  if (stall_disambig) {
    ...
    switch (LQ[lq_index].size) {
      ...
      case 8: LQ[lq_index].value = SQ[store_entry].value;
        break;
    ...
    try {
      switch (LQ[lq_index].size) {
        ...
        case 8: LQ[lq_index].value = (reg_t) mmu->load_uint64(LQ[lq_index].addr);
          break;
      ...
      ifprintf(logging_on, proc->lsu_log, "Cycle %"
          PRIcycle
          ": core %3d: load exception %s, epc 0x%016"
          PRIx64
          " badvaddr 0x%16"
          PRIx64
          "\n",
      ...
      switch (t.cause()) {
        case CAUSE_FAULT_LOAD:proc->set_exception(al_index);
          proc->PAY.buf[LQ[lq_index].pay_index].trap = new trap_load_access_fault(t.get_badvaddr());
          break;
        case CAUSE_MISALIGNED_LOAD:proc->set_exception(al_index);
    ...
    if (atomic_op && (proc->get_state()->load_reservation != SQ[sq_head].addr)) {
      ...
      try {
        ...
        switch (SQ[sq_head].size) {
          case 1:mmu->store_uint8(SQ[sq_head].addr, SQ[sq_head].value);
            break;
          case 2:mmu->store_uint16(SQ[sq_head].addr, SQ[sq_head].value);
            break;
          case 4:mmu->store_uint32(SQ[sq_head].addr, SQ[sq_head].value);
            break;
          case 8:mmu->store_uint64(SQ[sq_head].addr, SQ[sq_head].value);
            break;
          default:assert(0);
            break;
        }
...
void lsu::dump_lq(pipeline_t *proc, unsigned int index, FILE *file) {
  proc->disasm(proc->PAY.buf[LQ[index].pay_index].inst,
               proc->cycle,
               proc->PAY.buf[LQ[index].pay_index].pc,
               proc->PAY.buf[LQ[index].pay_index].sequence,
               file);
  ...
  ifprintf(logging_on, file, "addr       : %"
      PRIxreg
      "\t", LQ[index].addr);
  ifprintf(logging_on, file, "value_avail: %u\t", LQ[index].value_avail);
  ifprintf(logging_on, file, "value      : %"
      PRIxreg
      "\t", LQ[index].value);
  ifprintf(logging_on, file, "value      : %"
      PRIreg
  ...
  ifprintf(logging_on, file, "resolves   : %"
      PRIcycle
      "\t", LQ[index].miss_resolve_cycle);
...
void lsu::dump_sq(pipeline_t *proc, unsigned int index, FILE *file) {
  proc->disasm(proc->PAY.buf[SQ[index].pay_index].inst,
               proc->cycle,
               proc->PAY.buf[SQ[index].pay_index].pc,
               proc->PAY.buf[SQ[index].pay_index].sequence,
               file);
