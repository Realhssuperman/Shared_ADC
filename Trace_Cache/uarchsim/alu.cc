...
//
    ...
    case OP_JAL:PAY.buf[index].C_value.dw = INCREMENT_PC(PAY.buf[index].pc);
      PAY.buf[index].c_next_pc = (PAY.buf[index].pc + inst.uj_imm());  //uj_imm is already sign extended
      break;

    case OP_JALR:PAY.buf[index].C_value.dw = INCREMENT_PC(PAY.buf[index].pc);
      PAY.buf[index].c_next_pc = ((PAY.buf[index].A_value.dw + inst.i_imm()) &
          ~reg_t(1)); // Zeroes the LSB bit as per ISA
      break;
      ...
      switch (inst.funct3()) {
        case FN3_BEQ:
          PAY.buf[index].c_next_pc = (PAY.buf[index].A_value.dw == PAY.buf[index].B_value.dw) ?
                                     PAY.buf[index].pc + inst.sb_imm()
                                                                                              : //sb_imm is already a multiple of 2 PC target
        ...
        case FN3_BLT:
          PAY.buf[index].c_next_pc = (PAY.buf[index].A_value.sdw < PAY.buf[index].B_value.sdw)
                                     ? //Use signed values
                                     PAY.buf[index].pc + inst.sb_imm()
                                     : //sb_imm is already a multiple of 2 PC target
                                     INCREMENT_PC(PAY.buf[index].pc);
...
//
    ...
    case OP_OP_IMM:
      switch (inst.funct3()) {
        //ADDI
        case FN3_ADD_SUB:PAY.buf[index].C_value.sdw = PAY.buf[index].A_value.sdw + inst.i_imm();
        ...
        case FN3_SLT:
          ...
          break;
          //SLTIU
        case FN3_SLTU:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw < reg_t(inst.i_imm()) ? reg_t(1) : reg_t(0);
          break;
          //XORI
        case FN3_XOR:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw ^ inst.i_imm();
          break;
          //ORI
        case FN3_OR:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw | inst.i_imm();
          break;
          //ANDI
        case FN3_AND:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw & inst.i_imm();
          break;
          //SLLI
        case FN3_SLL:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw << inst.shamt();
          break;
        case FN3_SR:
          //SRLI
    ...
    case OP_OP_IMM_32:
      switch (inst.funct3()) {
        ...
        case FN3_ADD_SUB:
          ...
          break;
          //SLLIW
        case FN3_SLL:PAY.buf[index].C_value.sdw = sext32(PAY.buf[index].A_value.dw << (inst.shamt() & 0x1f));
          break;
        case FN3_SR:
          //SRLIW
          // Logical right shift, hence using unsigned operands
    ...
    case OP_OP:
      if (inst.funct7() == FN7_MULDIV) {
        switch (inst.funct3()) {
          //MUL
          case FN3_MUL:PAY.buf[index].C_value.sdw = sreg_t(PAY.buf[index].A_value.dw * PAY.buf[index].B_value.dw);
            break;
            //MULH
          case FN3_MULH:PAY.buf[index].C_value.sdw = mulh(PAY.buf[index].A_value.dw, PAY.buf[index].B_value.dw);
            break;
            //MULHSU
          case FN3_MULHSU:PAY.buf[index].C_value.sdw = mulhsu(PAY.buf[index].A_value.dw, PAY.buf[index].B_value.dw);
            //MULHU
          case FN3_MULHU:PAY.buf[index].C_value.sdw = mulhu(PAY.buf[index].A_value.dw, PAY.buf[index].B_value.dw);
            break;
            //DIV
          case FN3_DIV: {
        ...
        switch (inst.funct3()) {
          case FN3_ADD_SUB:
            ...
            break;
            //SLL
          case FN3_SLL:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw << (PAY.buf[index].B_value.dw & 0x3f);
            break;
            //SLT
          case FN3_SLT:
            PAY.buf[index].C_value.dw = (PAY.buf[index].A_value.sdw < PAY.buf[index].B_value.sdw) ? reg_t(1)
                                                                                                  : reg_t(
                    0);
            break;
            //SLTU
          case FN3_SLTU:
            PAY.buf[index].C_value.dw =
                (PAY.buf[index].A_value.dw) < (PAY.buf[index].B_value.dw) ? reg_t(1) : reg_t(0);
            break;
            //XOR
          case FN3_XOR:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw ^ PAY.buf[index].B_value.dw;
            break;
          case FN3_SR:
            //SRL
            // Logical right shift, hence using unsigned operands
            if (inst.funct7() == FN7_SRL) {
              ...
              PAY.buf[index].C_value.sdw =
                  PAY.buf[index].A_value.sdw >> (PAY.buf[index].B_value.dw & 0x3f);
            }
              //Undefined instruction
            else {
              assert(0);
            ...
            break;
            //OR
          case FN3_OR:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw | PAY.buf[index].B_value.dw;
            break;
            //AND
          case FN3_AND:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw & PAY.buf[index].B_value.dw;
            break;
        }
      }
      break;//OP_OP
    ...
    case OP_OP_32:
      if (inst.funct7() == FN7_MULDIV) {
        switch (inst.funct3()) {
          //MULW
          case FN3_MUL:PAY.buf[index].C_value.sdw = sext32(PAY.buf[index].A_value.dw * PAY.buf[index].B_value.dw);
            break;
            //DIVW
          case FN3_DIV: {
...
//
    ...
    case OP_LUI:PAY.buf[index].C_value.dw = inst.u_imm();
      break;

    case OP_AUIPC:PAY.buf[index].C_value.dw = PAY.buf[index].pc + inst.u_imm();
      break;
    ...
    case OP_NMSUB:
      ...
      if (valid_debug_index(PAY.buf[index].db_index)) {
        actual = pipe->peek(
            PAY.buf[index].db_index);      // Pointer to corresponding instruction in the functional simulator.
...
//
    case OP_OP_FP:
      ...
      if (inst.fmt() == FMT_S) {
        ...
        switch (inst.funct5()) {
          case FN5_FADD:PAY.buf[index].C_value.f[0] = PAY.buf[index].A_value.f[0] + PAY.buf[index].B_value.f[0];
            break;

          case FN5_FSUB:PAY.buf[index].C_value.f[0] = PAY.buf[index].A_value.f[0] - PAY.buf[index].B_value.f[0];
            break;

          case FN5_FMUL:PAY.buf[index].C_value.f[0] = PAY.buf[index].A_value.f[0] * PAY.buf[index].B_value.f[0];
            break;

          case FN5_FDIV:PAY.buf[index].C_value.f[0] = PAY.buf[index].A_value.f[0] / PAY.buf[index].B_value.f[0];
            break;
          ...
          case FN5_FCVT_I2FP:
            switch (inst.rs2()) {
              case RS2_FCVT_W:PAY.buf[index].C_value.f[0] = (float) PAY.buf[index].A_value.sw[0];
                break;

              case RS2_FCVT_WU:PAY.buf[index].C_value.f[0] = (float) PAY.buf[index].A_value.w[0];
                break;

              case RS2_FCVT_L:PAY.buf[index].C_value.f[0] = (float) PAY.buf[index].A_value.sdw;
                break;

              case RS2_FCVT_LU:PAY.buf[index].C_value.f[0] = (float) PAY.buf[index].A_value.dw;
                break;
            }
            break;
          ...
          case FN5_FCVT_FP2I:
            switch (inst.rs2()) {
              case RS2_FCVT_W:PAY.buf[index].C_value.sdw = sext32((sword_t) PAY.buf[index].A_value.f[0]);
                break;

              case RS2_FCVT_WU:PAY.buf[index].C_value.dw = (reg_t) PAY.buf[index].A_value.f[0];
                break;

              case RS2_FCVT_L:PAY.buf[index].C_value.sdw = (sreg_t) PAY.buf[index].A_value.f[0];
                break;

              case RS2_FCVT_LU:PAY.buf[index].C_value.dw = (reg_t) PAY.buf[index].A_value.f[0];
                break;
            ...
            // FCVT.S.D
          case FN5_FCVT_DS:PAY.buf[index].C_value.f[0] = (float) PAY.buf[index].A_value.d;
            break;

          case FN5_FCOMP:
            switch (inst.funct3()) {
              case FN3_FEQ:
                PAY.buf[index].C_value.w[0] = (PAY.buf[index].A_value.f[0] ==
                    PAY.buf[index].B_value.f[0]);
                break;
          ...
          case FN5_FSQRT:PAY.buf[index].C_value.f[0] = sqrt((double) PAY.buf[index].A_value.f[0]);
            break;
        ...
        switch (inst.funct5()) {
          case FN5_FADD:PAY.buf[index].C_value.d = PAY.buf[index].A_value.d + PAY.buf[index].B_value.d;
            break;

          case FN5_FSUB:PAY.buf[index].C_value.d = PAY.buf[index].A_value.d - PAY.buf[index].B_value.d;
            break;

          case FN5_FMUL:PAY.buf[index].C_value.d = PAY.buf[index].A_value.d * PAY.buf[index].B_value.d;
            break;

          case FN5_FDIV:PAY.buf[index].C_value.d = PAY.buf[index].A_value.d / PAY.buf[index].B_value.d;
            break;

          case FN5_FMV_I2FP:PAY.buf[index].C_value.dw = PAY.buf[index].A_value.dw;
            break;
          ...
          case FN5_FCVT_I2FP:
            switch (inst.rs2()) {
              case RS2_FCVT_W:PAY.buf[index].C_value.d = (double) PAY.buf[index].A_value.sw[0];
                break;

              case RS2_FCVT_WU:PAY.buf[index].C_value.d = (double) PAY.buf[index].A_value.w[0];
                break;

              case RS2_FCVT_L:PAY.buf[index].C_value.d = (double) PAY.buf[index].A_value.sdw;
                break;

              case RS2_FCVT_LU:PAY.buf[index].C_value.d = (double) PAY.buf[index].A_value.dw;
                break;
            }
            break;
          ...
          case FN5_FCVT_FP2I:
            switch (inst.rs2()) {
              case RS2_FCVT_W:PAY.buf[index].C_value.dw = sext32(PAY.buf[index].A_value.d);
                break;

              case RS2_FCVT_WU:PAY.buf[index].C_value.dw = zext32(PAY.buf[index].A_value.d);
                break;

              case RS2_FCVT_L:PAY.buf[index].C_value.sdw = (sreg_t) PAY.buf[index].A_value.d;
                break;

              case RS2_FCVT_LU:PAY.buf[index].C_value.dw = (reg_t) PAY.buf[index].A_value.d;
                break;
            }
            break;

            //FCVT.D.S
          case FN5_FCVT_DS:PAY.buf[index].C_value.d = (double) PAY.buf[index].A_value.f[0];
            break;

          case FN5_FCOMP:
            switch (inst.funct3()) {
              case FN3_FEQ:PAY.buf[index].C_value.dw = (PAY.buf[index].A_value.d == PAY.buf[index].B_value.d);
                break;

              case FN3_FLT:PAY.buf[index].C_value.dw = (PAY.buf[index].A_value.d < PAY.buf[index].B_value.d);
                break;

              case FN3_FLE:PAY.buf[index].C_value.dw = (PAY.buf[index].A_value.d <= PAY.buf[index].B_value.d);
                break;
            }
            break;

          case FN5_FSQRT:PAY.buf[index].C_value.d = sqrt(PAY.buf[index].A_value.d);
            break;
