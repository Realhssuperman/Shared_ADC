...
#include "trace_cache.h"   // add trace cache
...
//
  ...
  void disasm(insn_t insn,
              cycle_t cycle,
              reg_t pc,
              uint64_t seq,
              FILE *out = stderr); // disassemble and print an instruction
...
//
 ...
 private:
  ...
  // Indexed by FU type: bit vector indicating which lanes have that FU type.
  unsigned int fu_lane_matrix[(unsigned int) NUMBER_FU_TYPES];

  // Indexed by FU type: lane to which the last instruction of that FU type was steered.
  unsigned int fu_lane_ptr[(unsigned int) NUMBER_FU_TYPES];
