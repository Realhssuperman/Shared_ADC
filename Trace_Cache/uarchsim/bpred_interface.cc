...
static inline uint32_t calc_bp_tbl_idx(uint32_t history, uint32_t pc) {
  uint32_t idx;

#ifdef BHR_RSHIFT
  idx = ((history & HIST_MASK) ^
      ((pc / insn_size) & PC_MASK));
  // if BHR is built using RSHIFT mode, truncate the right most value to retain the most recent branch history
  if (BP_INDEX_WIDTH < BHR_LEN) {
    idx = (idx >> (BHR_LEN - BP_INDEX_WIDTH)) & BP_INDEX_MASK;
  }
#else

  static_assert(PC_MASK == 0x3fffu, "If PC_MASK != 0x3fffu, change the PC_LSHIFT_BIT accordingly");
  static const unsigned int PC_LSHIFT_BIT = 2;

  idx = ((history & HIST_MASK) ^
      (((pc / insn_size) & PC_MASK)) << PC_LSHIFT_BIT);

  // if BHR is built using LSHIFT mode, truncate the left most value to retain the most recent branch history
  idx = idx & BP_INDEX_MASK;
#endif
...
//  pred_table = new BpredPredictAutomaton[BP_TABLE_SIZE];
  ...
  // allocate and init the multiple prediction related structure
  multi_pred_table = new BpredPredictAutomaton *[MULTI_PRED_NUM_PRED];
  for (i = 0; i < MULTI_PRED_NUM_PRED; i++) {
    multi_pred_table[i] = new BpredPredictAutomaton[BP_TABLE_SIZE];
  }
  multi_pred_buf = new Future_Br_Pred_t[MULTI_PRED_NUM_PRED];

  // init the multiple predictor
  for (size_t x = 0; x < MULTI_PRED_NUM_PRED; ++x) {
    for (i = 0; i < BP_TABLE_SIZE; i++) {
      multi_pred_table[x][i].tag = 0;
      multi_pred_table[x][i].pred = 1;
      multi_pred_table[x][i].hyst = 0;
    }
  }
//  for (i = 0; i < MULTI_PRED_NUM_PRED; ++i) {
//    multi_pred_buf[i].taken = false;
//    multi_pred_buf[i].pred_col = 0;
//    multi_pred_buf[i].pred_index = 0;
//  }
//  multi_pred_table_trail_train_buf_tail = 0;
...
void bpred_interface::update() {
  ...
  if (cti_Q[current].is_cond) {
    if (cti_Q[current].taken) {
      cti_Q[new_tail].history = gen_next_history(cti_Q[current].history, true);
    } else {
      cti_Q[new_tail].history = gen_next_history(cti_Q[current].history, false);
    }
  } else {
...
void bpred_interface::update_predictions(bool fm)        // "FM"
{
  ...
  if (cti_Q[cti_head].is_cond) {
    ...
    // TODO: update multi pred table using the index we saved, because it might hashed using the PC of an instruction before the branch
    pred_index = cti_Q[cti_head].multi_bp_tbl_index;
    pred_col = cti_Q[cti_head].multi_bp_tbl_col;
    if (!cti_Q[cti_head].taken) {
      pred_col = cti_Q[cti_head].multi_bp_tbl_col;
    }
    multi_pred_table[pred_col][pred_index].update((uint32_t) cti_Q[cti_head].taken);
    ...
    if (cti_Q[cti_head].original_pred != cti_Q[cti_head].pc + 8) {
      ...
      history = gen_next_history(history, false);
    }

    conf_index = calc_bp_tbl_idx(history, cti_Q[cti_head].pc);
...
void bpred_interface::decode() {
  ...
  switch (inst.opcode()) {
    ...
    case OP_BRANCH : cti_Q[cti_tail].is_cond = true;
      break;
  ...
  switch (inst.opcode()) {
    case OP_BRANCH :
      if (next_pc != INCREMENT_PC(pc)) { // if taken
        ghistory = gen_next_history(ghistory, true);
      } else {
        ghistory = gen_next_history(ghistory, false);
      }
      return (ghistory);
      break;
    default: return (ghistory);
      break;
  }
}

bool bpred_interface::multi_pred_make_predictions(unsigned int branch_history) {
  uint32_t conf_index;
  uint32_t pred_index;
  uint32_t history;
  ...
  // Predict if cti is "taken"
  //
  assert(branch_history == 0xFFFFFFFF);
  assert(cti_Q[cti_tail].is_cond);
  assert(!cti_Q[cti_tail].is_return);
  assert(!cti_Q[cti_tail].is_call);
  assert(!cti_Q[cti_tail].use_BTB);

  history = cti_Q[cti_tail].history;

  pred_index = calc_bp_tbl_idx(cti_Q[cti_tail].history, cti_Q[cti_tail].pc);

  // TODO: get prediction from prediction buffer which is updated at the beginning of fetch, if we still have available prediction

  if (multi_pred_buf_avail_idx >= MULTI_PRED_NUM_PRED) {
    return false;
  }

  cti_Q[cti_tail].multi_bp_tbl_col = multi_pred_buf_avail_idx;
  cti_Q[cti_tail].multi_bp_tbl_index = multi_pred_buf[multi_pred_buf_avail_idx].pred_index;
  cti_Q[cti_tail].taken = multi_pred_buf[multi_pred_buf_avail_idx].taken;
  ...
  conf_index = calc_bp_tbl_idx(history, cti_Q[cti_tail].pc);
  ...
  multi_pred_buf_avail_idx++;

  return true;
}

// do multiple branch prediction
unsigned int bpred_interface::get_multi_pred_for_branch(unsigned int branch_history,
                                                        unsigned int PC,
                                                        insn_t inst,
  ...
  decode();
  *pred_vaild = multi_pred_make_predictions(branch_history);

  // If we use up all the prediction we make at the beginning of fetch stage,
  // we should stop here and don't update cti_tail
  if (!*pred_vaild) {
    return PC; // we will re-fetch this branch at next cycle
  }

  cti_Q[cti_tail].original_pred = cti_Q[cti_tail].target;

  *conf = cti_Q[cti_tail].conf > CONF_THRESHOLD;
  *fm = cti_Q[cti_tail].fm <= FM_THRESHOLD;// "FM"
...
void bpred_interface::prepare_multi_pred_future_buf(unsigned int PC, unsigned int *pred_vec) {
  uint32_t pred_index;

  pred_index = calc_bp_tbl_idx(cti_Q[cti_tail].history, PC);

  unsigned int tmp = 0;
  // TODO: update the multiple prediction buffer, and calc the pred_vec for the trace hit logic in future
  for (size_t i = 0; i < MULTI_PRED_NUM_PRED; i++) {
    multi_pred_buf[i].taken = multi_pred_table[i][pred_index].pred;
    multi_pred_buf[i].pred_index = pred_index; // need to save index for training
    if (multi_pred_buf[i].taken) {
      tmp |= (1u << i);
    }
  }
  *pred_vec = tmp;
  multi_pred_buf_avail_idx = 0;
}
