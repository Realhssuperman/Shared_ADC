...
void pipeline_t::rename2() {
  unsigned int i;
  unsigned int index;
  unsigned int n_checkpoint_needed, n_dest_reg_needed;
  ...
  if (REN->stall_reg(n_dest_reg_needed) || REN->stall_branch(n_checkpoint_needed)) {
    return;
  }
