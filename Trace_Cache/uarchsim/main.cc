...
static void help() {
  ...
  fprintf(stderr, "  --tc_en=<0/1>      Enable trace cache\n");
  fprintf(stderr, "  --tc_prop_m=<n>    Set M value (Maximum branch per trace). Note *the number prediction of multiple branch predictor* is also controlled by this parameter\n");
  fprintf(stderr, "  --tc_prop_n=<n>    Set N value (Maximum instruction per trace)\n");
  fprintf(stderr, "  --tc_capacity=<n>  Overall capacity, in number of trace, must be power of 2\n");
  fprintf(stderr, "  --tc_assoc=<n>     Associativity, must be power of 2\n");
  fprintf(stderr, "  --tc_heur=<0/1>    0 - No Heuristic applied; 1 - Terminate at backward jump\n");
  fprintf(stderr, "  --tc_max_para_fill=<n>\tNumber of parallel fill allowed\n");
  fprintf(stderr, "  --tc_non_blk_fill=<0/1>\tEnable non-block fill\n");
  fprintf(stderr, "  --tc_idx_with_pred_vec=<0/1>\tEnable path hash (hash PC with prediction bits to generate T$ index)\n");
...
static void set_lane_matrix(const char *config) {
  ...
  fprintf(stderr,
          "Lane Matrix: 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
          FU_LANE_MATRIX[0],
          FU_LANE_MATRIX[1],
          FU_LANE_MATRIX[2],
          FU_LANE_MATRIX[3],
...
static void set_perfect_flags(const char *config) {
  ...
  if (sscanf(config, "%lu,%lu,%lu,%lu", &pbp, &pdc, &pic, &ptc) != 4) {
    ...
    fprintf(stderr,
            "...where pbp (perfect branch prediction), pdc (perfect D$), pic (perfect I$), and ptc (perfect T$) are each 0 or 1.\n");
    exit(-1);
  } else {
...
static void set_disambig_flags(const char *config) {
  ...
  if (sscanf(config, "%lu,%lu", &omd, &smd) != 2) {
    ...
    fprintf(stderr,
            "...where oracle (oracle memory disambig.) and spec (speculative memory disambig.), are each 0 or 1\n");
    exit(-1);
  } else {
...
int main(int argc, char **argv) {
  ...
  parser.option('s', 0, 1, [&](const char *s) {
    skip_amt = atoll(s);
    skip_enable = true;
  });
  parser.option('e', 0, 1, [&](const char *s) {
    stop_amt = atoll(s);
    use_stop_amt = true;
  });
  ...
  parser.option(0, "btb", 1, [&](const char *s) {
    BTB_SIZE = atoi(s);
    BTB_MASK = BTB_SIZE - 1;
  });
  parser.option(0, "ctiq", 1, [&](const char *s) {
    CTIQ_SIZE = atoi(s);
    CTIQ_MASK = CTIQ_SIZE - 1;
  });
  parser.option(0, "bp", 1, [&](const char *s) {
    BP_TABLE_SIZE = atoi(s);
    BP_INDEX_MASK = BP_TABLE_SIZE - 1;
    BP_INDEX_WIDTH = (unsigned int) ceil(log2(BP_TABLE_SIZE));
  });
  ...
  parser.option(0, "lsq", 1, [&](const char *s) {
    LQ_SIZE = atoi(s);
    SQ_SIZE = atoi(s);
  });
  ...
  parser.option(0, "tc_en", 1, [&](const char *s) { TRACE_CACHE_ENABLE = atoi(s); });
  parser.option(0, "tc_prop_m", 1, [&](const char *s) {
    TRACE_CACHE_PROP_M = atoi(s);
    MULTI_PRED_NUM_PRED = atoi(s);
  });
  parser.option(0, "tc_prop_n", 1, [&](const char *s) { TRACE_CACHE_PROP_N = atoi(s); });
  parser.option(0, "tc_capacity", 1, [&](const char *s) { TRACE_CACHE_CAPACITY = atoi(s); });
  parser.option(0, "tc_assoc", 1, [&](const char *s) { TRACE_CACHE_ASSOC = atoi(s); });
  parser.option(0, "tc_heur", 1, [&](const char *s) { TRACE_CACHE_TERM_HEURISTIC = atoi(s); });
  parser.option(0, "tc_max_para_fill", 1, [&](const char *s) { TRACE_CACHE_MAX_ONGOING_FILL = atoi(s); });
  parser.option(0, "tc_non_blk_fill", 1, [&](const char *s) { TRACE_CACHE_NON_BLOCK_FILL = atoi(s); });
  parser.option(0, "tc_idx_with_pred_vec", 1, [&](const char *s) { TRACE_INDEX_WITH_PRED_BIT = atoi(s); });
