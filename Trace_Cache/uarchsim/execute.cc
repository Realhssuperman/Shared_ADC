  ...
  if (Execution_Lanes[lane_number].ex[depth].valid) {
    ...
    if (IS_MEM_OP(PAY.buf[index].flags)) {
      ...
      if (IS_LOAD(PAY.buf[index].flags)) {
        ...
        if (PAY.buf[index].split_store) {
          ...
          if (PAY.buf[index].upper)
            LSU.store_addr(cycle,
                           PAY.buf[index].addr,
                           PAY.buf[index].SQ_index,
                           PAY.buf[index].LQ_index,
                           PAY.buf[index].LQ_phase);    // upper op: address
          else
      ...
      try {
        ...
        ifprintf(logging_on, execute_log, "Cycle %"
            PRIcycle
            ": core %3d: exception %s, epc 0x%016"
            PRIx64
        ...
        ifprintf(logging_on, execute_log, "Cycle %"
            PRIcycle
            ": core %3d: exception refernce thrown from unknown source %s, epc 0x%016"
            PRIx64
            " al_index %u\n", cycle, id, t.name(), epc, al_index);
        trap_t *tp;
        switch (t.cause()) {
          case CAUSE_FP_DISABLED:tp = new trap_fp_disabled();
            break;
          case CAUSE_ILLEGAL_INSTRUCTION:tp = new trap_illegal_instruction();
            break;
          case CAUSE_PRIVILEGED_INSTRUCTION:tp = new trap_privileged_instruction();
            break;
          default:fflush(0);
            assert(0);
            break;
